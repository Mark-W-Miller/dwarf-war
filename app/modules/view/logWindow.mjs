...
export function initLogWindow({ Log }) {
  ...
  let savedOpen = false;
  savedOpen = (localStorage.getItem(OPEN_KEY) === '1');
  ...
  (function enableDrag(){
    ...
    function onMove(e){
      ...
      localStorage.setItem(POS_KEY, JSON.stringify({ right: rightPx, top: topPx }));
    }
    function onUp(){ dragging=false; document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp);  const r = parseInt(root.style.right)||24, t = parseInt(root.style.top)||64; localStorage.setItem(POS_KEY, JSON.stringify({ right: r, top: t }));  }
 })();
  ...
  const api = {
    open(){ localStorage.setItem(OPEN_KEY, '1');  root.style.display='block'; renderFilters(); renderEntries(); },
    close(){ localStorage.setItem(OPEN_KEY, '0');  root.style.display='none'; },
  ...
  if (savedOpen) { api.open(); } else { api.close();  }
  return api;
}
