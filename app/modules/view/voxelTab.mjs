...
export function initVoxelTab(panelContent, api) {
  ...
  tabBtn.addEventListener('click', () => { Log.log('UI', 'Activate tab', { tab: 'Voxel' }); });
  ...
  function renderSelection() {
    ...
    const vName = (val) => { return Object.keys(VoxelType).find(k => VoxelType[k] === val) || String(val); };
    ...
    if (spaces.length > 1) {
      ...
      if (voxed.length >= 2) {
        ...
        let timer = null; const schedule = () => { if (timer) clearTimeout(timer); timer = setTimeout(() => { api.rebuildScene?.(); ; timer = null; }, 60); };
        const applyGroup = (targetVal) => {
          ...
          for (const b of bases) { b.s.voxExposeTop = Math.max(0, Math.min(b.vy, (b.e + delta)|0));  }
          schedule();
 };
    ...
    if (s.vox && s.vox.size) {
      ...
      const len = Array.isArray(s.vox.data) ? s.vox.data.length : (s.vox.data?.rle?.length || 0); makeRow('data (len)', String(len));
      ...
      const update = (v, doRebuild = false) => { const nv = clamp(v); range.value = String(nv); num.value = String(nv);  s.voxExposeTop = nv; ; if (doRebuild) schedule(); };
  ...
  bakeBtn.addEventListener('click', async () => {
    ...
    window.dispatchEvent(new CustomEvent('dw:gizmos:disable'));
    ...
    api.saveBarrow(api.state.barrow); api.snapshot(api.state.barrow);
    api.renderDbView(api.state.barrow);
    api.rebuildScene?.();
    api.scheduleGridUpdate?.();
    window.dispatchEvent(new CustomEvent('dw:transform', { detail: { kind: 'voxel-bake', sel: spaces.map(s => s.id), wallThickness: t } }));
    window.dispatchEvent(new CustomEvent('dw:gizmos:enable'));
 });
  ...
  ovoidBtn.addEventListener('click', async () => {
    ...
    api.rebuildScene?.();
    api.scheduleGridUpdate?.();
    window.dispatchEvent(new CustomEvent('dw:transform', { detail: { kind: 'voxel-bake-ovoid', sel: spaces.map(s => s.id), wallThickness: t } }));
    window.dispatchEvent(new CustomEvent('dw:gizmos:enable'));
 });

  // New: explicit Merge button handler (union merge of overlapping spaces)
  let _cancelMerge = { value: false };
  cancelBtn.addEventListener('click', () => { if (!cancelBtn.disabled) { _cancelMerge.value = true;  Log.log('UI', 'Cancel voxel op', {}); } });
  ...
  mergeBtn.addEventListener('click', async () => {
    ...
    const debugCfg = {
      chunk: 256,
      // Progress + terse logs
      onStart: ({ nx, ny, nz }) => { Log.log('DEBUG', 'Union scan start', { ny }); ; _nyTotal = ny||0; _showProg(); _setProgFromLayer(-1); },
      onLayer: (y) => { _setProgFromLayer(y); },
      onEnd: () => { if (scanDotsOn) api.debug?.flushVoxelScanPoints?.(); Log.log('DEBUG', 'Union scan end', {}); ; _hideProg(); },
      showObb: (corners) => { api.debug?.showObbDebug?.(corners);  }
 };
    // If currently enabled, prepare base meshes
    if (scanDotsOn) api.debug?.startVoxelScanDebug?.(res);
    // Always provide callbacks; gate with scanDotsOn so toggling works mid-run
    debugCfg.onTestInside = (wx, wy, wz) => { if (scanDotsOn) { api.debug?.addVoxelScanPointInside?.(wx, wy, wz); } };
    debugCfg.onTestOutside = (wx, wy, wz) => { if (scanDotsOn) { api.debug?.addVoxelScanPointOutside?.(wx, wy, wz); } };
    debugCfg.flush = () => { if (scanDotsOn) { api.debug?.flushVoxelScanPoints?.(); } };
    ...
    if (!_cancelMerge.value) { const id = keepId || seed.id; api.state.selection.clear(); api.state.selection.add(id); }
    api.saveBarrow(api.state.barrow); api.snapshot(api.state.barrow);
    api.renderDbView(api.state.barrow);
    api.rebuildScene?.();
    api.scheduleGridUpdate?.();
    if (!_cancelMerge.value) window.dispatchEvent(new CustomEvent('dw:transform', { detail: { kind: 'voxel-merge', sel: [seed.id], keepId: keepId||seed.id } }));
    window.dispatchEvent(new CustomEvent('dw:gizmos:enable'));
    _hideProg();
  ...
  softMergeBtn.addEventListener('click', async () => {
    ...
    const debugCfg = { chunk: 256,
      onStart: ({ ny }) => { _nyTotal = ny||0; _setProgFromLayer(-1); if (scanDotsOn) { api.debug?.flushVoxelScanPoints?.(); } },
      onLayer: (y) => { _setProgFromLayer(y); if (scanDotsOn && (y % 2 === 0)) { api.debug?.flushVoxelScanPoints?.(); } },
      onEnd: () => { _hideProg();  if (scanDotsOn) api.debug?.flushVoxelScanPoints?.(); },
      onTestInside: (wx, wy, wz) => { if (scanDotsOn) { api.debug?.addVoxelScanPointInside?.(wx, wy, wz); } },
      onTestOutside: (wx, wy, wz) => { if (scanDotsOn) { api.debug?.addVoxelScanPointOutside?.(wx, wy, wz); } }
 };
    if (scanDotsOn)  api.debug?.startVoxelScanDebug?.(res);
    ...
    // Persist and refresh
    if (!_cancelMerge.value) { api.saveBarrow(api.state.barrow); api.snapshot(api.state.barrow); }
    api.renderDbView(api.state.barrow);
    api.rebuildScene?.();
    api.scheduleGridUpdate?.();
    if (!_cancelMerge.value) window.dispatchEvent(new CustomEvent('dw:transform', { detail: { kind: 'voxel-soft-merge', sel: spaces.map(s => s.id), seedId: seed.id, keepId } }));
    window.dispatchEvent(new CustomEvent('dw:gizmos:enable'));
    _hideProg();
    cancelBtn.disabled = true;
 });
  ...
  scanDotsCb.addEventListener('change', () => {
    scanDotsOn = !!scanDotsCb.checked;
    localStorage.setItem(key, scanDotsCb.checked ? '1' : '0');
  ...
  clearBtn.addEventListener('click', () => { api.debug?.endVoxelScanDebug?.(); Log.log('DEBUG', 'Cleared scan dots', {}); });
  ...
  clearObbBtn.addEventListener('click', () => { api.debug?.clearObbDebug?.(); Log.log('DEBUG', 'Clear OBB (button)', {}); });
  ...
  fillBtn.addEventListener('click', () => {
    ...
    api.rebuildScene?.();
    api.scheduleGridUpdate?.();
    window.dispatchEvent(new CustomEvent('dw:transform', { detail: { kind: 'voxel-fill', sel: spaces.map(s => s.id), value: 'Rock' } }));
    window.dispatchEvent(new CustomEvent('dw:gizmos:enable'));
 });
