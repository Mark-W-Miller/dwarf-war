...
function voxelHoverForSpace(routerState, space) {
  ...
  try { if (localStorage.getItem('dw:ui:hoverVoxel') !== '1') return false; } catch {}
  if (!space || !space.vox || !space.vox.size) return false;
  if (state.lockedVoxPick && state.lockedVoxPick.id === space.id) return false;
  routerState._voxHoverLast = routerState._voxHoverLast || 0;
  const now = (typeof performance !== 'undefined' && performance.now) ? performance.now() : Date.now();
  if (now - routerState._voxHoverLast < 25) return true;
  ...
  let q = BABYLON.Quaternion.Identity(); const worldAligned = !!(space.vox && space.vox.worldAligned);
  if (!worldAligned) {
  ...
  if (!(tmax >= Math.max(0, tmin))) return false;
    ...
    if (iy < yCut) {
      ...
      if (v !== 0) {
        ...
        if (!prev || prev.x !== ix || prev.y !== iy || prev.z !== iz) {
          ...
          log('HOVER', 'voxel:hover', { id: space.id, i: ix, j: iy, k: iz });
        }
        return true;
      }
    }
...
export function routerHandleHover(routerState) {
  ...
  if (gesture?.decision) {
    // Clear all hover visuals while dragging camera
    clearPPHover(routerState);
    // Gizmo clear first so PP restore isn't skipped by hover.kind changes
    // (PP hover is tracked independently in ppHover now)
    // but still make sure gizmo visuals reset
    const { hover } = routerState;
    if (hover?.kind === 'rotAxis' || hover?.kind === 'moveAxis' || hover?.kind === 'moveDisc') {
      if (hover.kind === 'rotAxis') dimRotRings(scene);
      if (hover.kind === 'moveAxis') resetMoveMat(hover.mat);
      if (hover.kind === 'moveDisc') resetDiscMat(hover.mat);
  ...
  if (pp && pp.pickedMesh) { clearSpaceHover(routerState); return setHoverPPNode(routerState, pp.pickedMesh); }
  // 2) Gizmo parts
  const mg = pickMoveGizmo({ scene, x, y });
  if (mg && mg.pickedMesh) {
    const nm = String(mg.pickedMesh.name || '');
    if (nm.startsWith('moveGizmo:disc:')) { clearPPHover(routerState); clearSpaceHover(routerState); return setHoverMoveDisc(routerState, mg.pickedMesh); }
    if (nm.startsWith('moveGizmo:'))     { clearPPHover(routerState); clearSpaceHover(routerState); return setHoverMoveAxis(routerState, mg.pickedMesh); }
  }
  ...
  if (rg && rg.pickedMesh) {
    ...
    if (ax) { clearPPHover(routerState); clearSpaceHover(routerState); return setHoverRotAxis(routerState, ax); }
  }
  // 2.5) Spaces (prefer voxels when available)
  ...
  if (sp && sp.pickedMesh) {
    ...
    const space = (state?.barrow?.spaces || []).find(xx => xx && String(xx.id) === String(id)) || null;
    const hasVox = !!(space && space.vox && space.vox.size);
    if (hasVox) { clearPPHover(routerState); if (voxelHoverForSpace(routerState, space)) return; }
    if (!isSelected) { clearPPHover(routerState); return setHoverSpaceOutline(routerState, id); }
  }
  // 3) Miss — clear all
