...
export function buildEditTab({ state, Log } = {}) {
  ...
  // Row 1: Set voxel types + Connect controls
  ...
  const btnFinalize = document.createElement('button'); btnFinalize.className = 'btn'; btnFinalize.id = 'voxelConnectFinalize'; btnFinalize.textContent = 'Finalize Path'; btnFinalize.title = 'Commit current proposed path to tunnels'; btnFinalize.disabled = true;
  ...
  // Assemble
  section.appendChild(title); section.appendChild(row2); section.appendChild(row3);
  ...
  const dom = {
    ...
    btnTunnel: null, btnEmpty, btnRock, btnWall, btnConnect, btnFinalize, minInput,
};
