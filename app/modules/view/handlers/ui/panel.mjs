...
export function initPanelUI({ panel, collapsePanelBtn, engine, Log }) {
  ...
  function onMove(e){ if (!dragging) return; const dx = (startX - (e.clientX || 0)); let w = Math.round(startW + dx); const maxW = Math.max(260, window.innerWidth - 80); w = Math.max(240, Math.min(maxW, w)); panel.style.width = w + 'px';  localStorage.setItem('dw:ui:panelWidth', String(w));  }
  function onUp(){ if (!dragging) return; dragging = false;  document.removeEventListener('mousemove', onMove); document.removeEventListener('mouseup', onUp); document.removeEventListener('touchmove', onMove); document.removeEventListener('touchend', onUp);  }
  ...
  window.addEventListener('resize', () => { const maxW = Math.max(260, window.innerWidth - 80); const cur = panel.getBoundingClientRect().width; if (cur > maxW) { panel.style.width = maxW + 'px'; localStorage.setItem('dw:ui:panelWidth', String(maxW)); } });
  ...
  collapsePanelBtn?.addEventListener('click', () => { const next = !panel.classList.contains('collapsed'); applyPanelCollapsed(next);  localStorage.setItem(PANEL_STATE_KEY, next ? '1' : '0'); });
  ...
  logBtn?.addEventListener('click', () => { logWin.toggle(); });
