...
export function initScryApi({ scene, engine, camera, state, Log }) {
  ...
  function findScryWorldPosForSpace(space) {
    ...
    const q = (() => { const rx=Number(space.rotation?.x||0), ry=(typeof space.rotation?.y==='number')? Number(space.rotation.y):Number(space.rotY||0)||0, rz=Number(space.rotation?.z||0); return BABYLON.Quaternion.FromEulerAngles(rx, ry, rz); })();
  ...
  function enterScryMode() {
    ...
    state._scry._camKeys = { up: camera.keysUp?.slice(), down: camera.keysDown?.slice(), left: camera.keysLeft?.slice(), right: camera.keysRight?.slice() };
    camera.keysUp = []; camera.keysDown = []; camera.keysLeft = []; camera.keysRight = [];
    if (state.hl && ball) { state.hl.addMesh(ball, new BABYLON.Color3(0.4, 0.85, 1.0)); }
    if (ball) { ball.outlineColor = new BABYLON.Color3(0.35, 0.9, 1.0); ball.outlineWidth = 0.02; ball.renderOutline = true; }
    ...
    if (state._scry.scryKeys) window.removeEventListener('keydown', state._scry.scryKeys);
    if (state._scry.scryKeysUp) window.removeEventListener('keyup', state._scry.scryKeysUp);
    ...
    state._scry._onBlur = () => { state._scry.keyState = { up:false, down:false, left:false, right:false, shift:false, meta:false }; };
    ...
    state._scry.scryObs = engine.onBeginFrameObservable.add(() => {
      ...
      if (moveSign !== 0 && ball2) {
        ...
        let scryMult = 1.0;  const raw = Number(localStorage.getItem('dw:ui:scrySpeed') || '100'); if (isFinite(raw) && raw > 0) scryMult = (raw > 5) ? (raw/100) : raw;
        ...
        function canOccupy(px, py, pz) {
          ...
          for (const o of offsets) {
            const hit = (() => { const spaces = Array.isArray(state?.barrow?.spaces) ? state.barrow.spaces : []; for (const sp of spaces) { if (!sp || !sp.vox) continue; const v = voxelValueAtWorld(sp, px + o.x, py, pz + o.z); if (v === VoxelType.Rock || v === VoxelType.Wall) return true; } return false; })();
            if (hit) return false;
          }
          return true;
        }
        ...
        if (blocked) Log?.log('COLLIDE', 'scry:block', { from: pos, to: next, vert: isVert });
      }
  ...
  function exitScryMode() {
    if (!state._scry?.scryMode) return;
    const id = state._scry.spaceId; const b = state._scry.ball; if (id && b && !b.isDisposed()) localStorage.setItem('dw:scry:pos:'+id, JSON.stringify({ x: b.position.x, y: b.position.y, z:b.position.z }));
    state._scry.scryMode = false;
    if (state._scry.scryObs) { engine.onBeginFrameObservable.remove(state._scry.scryObs); state._scry.scryObs = null; }
    if (state._scry.scryKeys) { window.removeEventListener('keydown', state._scry.scryKeys); state._scry.scryKeys = null; }
    if (state._scry.scryKeysUp) { window.removeEventListener('keyup', state._scry.scryKeysUp); state._scry.scryKeysUp = null; }
    if (state._scry._onBlur) { window.removeEventListener('blur', state._scry._onBlur); state._scry._onBlur = null; }
    if (state._scry._camKeys) { camera.keysUp = state._scry._camKeys.up || camera.keysUp; camera.keysDown = state._scry._camKeys.down || camera.keysDown; camera.keysLeft = state._scry._camKeys.left || camera.keysLeft; camera.keysRight = state._scry._camKeys.right || camera.keysRight; state._scry._camKeys = null; }
    if (state.hl && state._scry?.ball) state.hl.removeMesh(state._scry.ball);
    if (state._scry?.ball) state._scry.ball.renderOutline = false;
    Log?.log('SCRY', 'exit', {});
